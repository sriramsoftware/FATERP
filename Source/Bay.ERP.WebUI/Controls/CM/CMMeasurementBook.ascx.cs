...
{
    public partial class CMMeasurementBookControl : BaseControl
    {
        #region Properties
        ...
        private void BuildDropDownList()
        {
            ...
            MiscUtil.PopulateMDVendorCategory(ddlVendorCategoryID, false);

            if (ddlVendorCategoryID.Items.Count > 0 && ddlVendorCategoryID != null)
            {
                ddlVendorCategoryID.SelectedValue = MasterDataConstants.CM_Bill_VendorCategory.CONTRACTOR;
            }

           // MiscUtil.PopulatePRMWorkOrder(ddlWorkOrderID, true);

            MiscUtil.PopulateMDItem(ddlItemID, false);
            BuildWorkOrderBySupplierID();
            BuildProjectCodeByProjectID();
            BuildWorkOrderItem();
            BuildVendorByVendorCategory();
        }

        private void BuildWorkOrderItem()
        {
            if (ddlWorkOrderID.Items.Count>0 && ddlWorkOrderID.SelectedValue != "0")
            {
                MiscUtil.PopulateMDItemByWorkOrder(ddlItemID, false, Int64.Parse(ddlWorkOrderID.SelectedValue));
                chbxItem.Enabled = true;
            }
            else if (ddlWorkOrderID.SelectedValue == "0" || ddlWorkOrderID.Items.Count<=0)
            {
                MiscUtil.PopulateMDItem(ddlItemID, false);
        ...
        private void BuildWorkOrderBySupplierID()
        {
            if (ddlSupplierID.Items.Count>0 && ddlSupplierID.SelectedValue != "0")
            {
                MiscUtil.PopulatePRMWorkOrderbySupplierID(ddlWorkOrderID, true, Int64.Parse(ddlSupplierID.SelectedValue.ToString()));
                BuildWorkOrderItem();
            }
        }
            ...
            if (ddlVendorCategoryID != null && ddlVendorCategoryID.Items.Count > 0)
            {
                switch (ddlVendorCategoryID.SelectedValue.ToString())
                {
                    ...
                    case MasterDataConstants.CM_Bill_VendorCategory.CONTRACTOR:
                        MiscUtil.PopulateCMContractor(ddlSupplierID, false);
                        break;
                    case MasterDataConstants.CM_Bill_VendorCategory.CONSULTANT:
                        MiscUtil.PopulateCMConsultant(ddlSupplierID, false);
                        break;
        ...
        private void PrepareEditView()
        {
            ...
            if (!cMMeasurementBookEntity.IsNew)
            {
                ...
                if (ddlVendorCategoryID.Items.Count > 0 && cMMeasurementBookEntity.VendorCategoryID != null)
                {
                    ddlVendorCategoryID.SelectedValue = cMMeasurementBookEntity.VendorCategoryID.ToString();
                }
        ...
        private CMMeasurementBookEntity BuildCMMeasurementBookEntity()
        {
            ...
            if (ddlVendorCategoryID.Items.Count > 0)
            {
                ...
                {
                    cMMeasurementBookEntity.VendorCategoryID = Int64.Parse(ddlVendorCategoryID.SelectedValue);
                }
            }
            ...
            if (IsValid)
            {
                try
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        if (cMMeasurementBookEntity.IsNew)
                        {
                            MiscUtil.ShowMessage(lblMessage, "Measurement Book Information has been added successfully.", false);
                        }
                        else
                        {
                            MiscUtil.ShowMessage(lblMessage, "Measurement Book Information has been updated successfully.", false);
                        }
                    }
                    else
                    {
                ...
                else if (string.Equals(e.CommandName, "DeleteItem"))
                {
                    try
                    {
                        ...
                        if (result == 0)
                        {
                            ...
                            MiscUtil.ShowMessage(lblMessage, "Measurement Book has been successfully deleted.", true);
                        }
                        else
                        {
                            MiscUtil.ShowMessage(lblMessage, "Failed to delete Measurement Book.", true);
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        protected void chbxItem_CheckedChanged(object sender, EventArgs e)
        {
            if (chbxItem.Checked == false)
            {
                BuildWorkOrderItem();
            }
            else if (chbxItem.Checked == true)
            {   
                MiscUtil.PopulateMDItem(ddlItemID, false);
            }
        }

...
namespace Bay.ERP.Web.UI
{
    public partial class INVInterProjectTransferReportControl : BaseControl
    {
        ...
        {
            lvACProjectWiseAccount.DataBind();
        }
        ...
        {
            String fe = String.Empty;//"INVTransfer_1.RequestDate between '" + MiscUtil.ParseToDateTime(txtStartDate.Text.Trim().ToString()).ToString() + "' AND '" + MiscUtil.ParseToDateTime(txtEndDate.Text.Trim().ToString()).ToString() + "'";

            if (ddlProjectID != null && ddlProjectID.SelectedValue != "0")
            {
                String fe2 = SqlExpressionBuilder.PrepareFilterExpression("BalanceSheetView.ProjectID", ddlProjectID.SelectedValue.ToString(), SQLMatchType.Equal);

                fe = SqlExpressionBuilder.PrepareFilterExpression(fe, SQLJoinType.AND, fe2);
            }
            if (hdEmpID.Value!="0")
            {
                String fe2 = SqlExpressionBuilder.PrepareFilterExpression("AccountID", hdEmpID.Value.ToString(), SQLMatchType.Equal);
